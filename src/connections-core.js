...
function syncCompanyRail(availableOwners,chosen){
 const container=$('#company-nodes');
 ownerList=availableOwners;
 const signature=`${state.region}|${availableOwners.map(g=>g.id).join(',')}`;
 if(container.dataset.ownerSignature!==signature){
  container.innerHTML=availableOwners.map(g=>`<button class="company-node" data-owner="${esc(g.id)}" data-port="owner:${esc(g.id)}" type="button" aria-pressed="false" aria-label="${esc(g.name)}, ${g.members.length} sites in ${esc(state.region)}"><span class="company-name">${esc(g.name)}</span><span class="company-count">${g.members.length}</span><span class="company-sub">${g.members.length} sites in scope</span></button>`).join('');
  container.dataset.ownerSignature=signature;
 }
 const relatedCounts=new Map();
 for(const plant of chosen){const id=ownCache.get(plant.id);if(id)relatedCounts.set(id,(relatedCounts.get(id)??0)+1);}
 const byId=new Map(availableOwners.map(g=>[g.id,g]));
 for(const node of $$('.company-node[data-owner]',container)){
  const id=node.dataset.owner,g=byId.get(id);if(!g)continue;
  const related=relatedCounts.get(id)??0;
  const select=state.filters.owner===id;
  const dim=isFocused()&&!related;
  node.classList.toggle('is-selected',select);
  node.classList.toggle('dim',dim);
  node.setAttribute('aria-pressed',String(select));
  node.setAttribute('aria-label',`${g.name}, ${g.members.length} sites in ${state.region}`);
  const count=node.querySelector('.company-count');if(count)count.textContent=String(g.members.length);
  const sub=node.querySelector('.company-sub');if(sub)sub.textContent=`${isFocused()?`${related} connected · `:''}${g.members.length} sites in scope`;
 }
...
function update(){
 ...
 const availableOwners=owners(base);
 syncCompanyRail(availableOwners,chosen);
...
$('#clear-all').onclick=clear;$('#brand-home').onclick=e=>{e.preventDefault();setRegion('World');window.scrollTo({top:0,behavior:'smooth'});};
