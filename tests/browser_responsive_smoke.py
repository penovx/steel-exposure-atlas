...
def preview_server():
    ...
    handler=lambda *a,**k: QuietHandler(*a,directory=str(ROOT),**k); server=socketserver.ThreadingTCPServer(('127.0.0.1',port),handler); thread=threading.Thread(target=server.serve_forever,daemon=True); thread.start()
    try: yield f'http://127.0.0.1:{port}/'
    finally: server.shutdown(); server.server_close(); thread.join(timeout=2)
def wait_for_atlas(page):
    page.wait_for_selector('#geo-map .land',state='attached',timeout=15000); page.wait_for_selector("#company-nodes[data-virtual-rail='true'] .company-node[data-virtual-owner]",state='attached',timeout=15000); page.wait_for_timeout(80)
