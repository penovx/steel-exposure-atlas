...
5. **Sanctions context** — direct entity-list matches against the **EU consolidated financial sanctions list**, with match evidence and snapshot date. A non-match is never presented as sanctions clearance because ownership/control rules can affect unlisted entities.
...
- a sanctions connection means a direct reviewed match to a listed entity or alias in a pinned EU sanctions-list snapshot;
